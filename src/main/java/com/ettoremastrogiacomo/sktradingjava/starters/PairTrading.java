     ...
     public static void main(String [] args) throws Exception {                  
         ...
         for (String x : map.keySet()){
             ...
             if (nmap.get(x).contains("STOCK")) {
                 if (maxt<map.get(x).size()) maxt=map.get(x).size();
         ...
         for (String i: timesmap.keySet()){
             if (timesmap.get(i)>(maxt*.7)) 
                 logger.debug(nmap.get(i)+"\t"+timesmap.get(i));         
             else toremove.add(i);
         }
         toremove.forEach((x)->{timesmap.remove(x);});
         ...
         for (String x : timesmap.keySet()){
             ...
             for (UDate d: td1) tmap1.put(d, Database.getIntradayFintsQuotes(x, d));
             double samples1=0;
             for (Fints f1: tmap1.values()) samples1+=f1.getLength();
         ...
         Misc.writeObjToFile(fintsmap, "prova.dat");
