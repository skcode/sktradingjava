...
public class DoubleDoubleArray {
    ...
    public static double[][] transpose(double[][] matrix) throws Exception {
        if (!isRectangular(matrix)) {
            throw new Exception("Not rectangular matrix");
        }
    ...
    static public double[][] minus(double[][] A, double[][] B) throws Exception {
        ...
        if (!isRectangular(A) || !isRectangular(B) || rA != rB || cA != cB) {
    ...
    static public double[][] times(double[][] A, double[][] B) throws Exception {
        int rA = nRows(A), cA = nCols(A), rB = nRows(B), cB = nCols(B);
        if (!isRectangular(A) || !isRectangular(B) || cA != rB) {
            throw new Exception("Illegal matrix dimensions.");
        }
    ...
    static public  boolean check_exists_le(double[][] m,double v) {
        boolean res=false;
        for (double[] m1 : m) {
            for (int j = 0; j < m1.length; j++) {
                if (m1[j]<=v) {
    ...
    static public  boolean check_exists_ge(double[][] m,double v) {
        boolean res=false;
        for (double[] m1 : m) {
            for (int j = 0; j < m1.length; j++) {
                if (m1[j]>=v) {
